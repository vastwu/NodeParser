...
const isSelfCloseTag = (function () {
  ...
  'area,base,br,col,command,embed,hr,img,input,keygen,link,meta,param,source,track,wbr,mpvoice,qqmusic'.split(',').forEach(function (key) {
...
const parseString = function (string) {
  ...
  while (cursor < len) {
    ...
    if (/['"]/.test(charset)) {
      //inQuate = charset
      // 如果是引号, 看是否匹配之前找到的引号开头
      if (!inQuate) {
        // 之前没有引号，此处是开始
        inQuate = charset
      } else if (inQuate && inQuate === charset) {
        // 之前有引号，并且一致，是结尾
        inQuate = false
      }
    }
...
const parseTag = function (list, parent) {
  ...
  while (list.length > 0) {
    ...
    if (begin) {
      ...
      if (/\/>$/.test(str) || isSelfCloseTag(node.tagName)) {
        node.isSelfClose = true
      } else {
        node.isSelfClose = false
      }
      if (!node.isSelfClose) {
        parseTag(list, node)
      }
